...
class FuncListController: UITableViewController {
  ...
  override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    case 1:
      ...
      case 5:
        ...
        break
      case 6:
        doWorkBackground("Import EOS WIF") {
            self.importEOSPrivateKey()
        }
        break
      default:
        break
  ...
  func transferEthToken() {
    do {
      ...
      let data = "0xa9059cbb\(Constants.password.removePrefix0xIfNeeded())\(BigNumber.parse("1000000000000000", padding: true).hexString())"
  ...
  func importEOSPrivateKey() {
    do {
        ...
        let permissions = [
            ...
            EOS.PermissionObject(permission: "active", publicKey: "EOS6qTGVvgoT39AAJp1ykty8XVDFv1GfW4QoS4VyjfQQPv5ziMNzF", parent: "owner")
        ]
        let eosWallet = try! WalletManager.importEOS(from: privateKeys, accountName: "", permissions: permissions, encryptedBy: Constants.password, metadata: WalletMeta(chain: .eos, source: .privateKey))
        
        let newEosWallet = try! WalletManager.changePassword(walletID: eosWallet.walletID, oldPassword: Constants.password, newPassword: "123456")
        
        let exportedKeys = try! WalletManager.exportPrivateKeys(walletID: newEosWallet.walletID, password: "123456").map { $0.privateKey }
        
        requestResult = "Import EOS Wallet by PrivateKey success:\n"
        requestResult = requestResult + prettyPrintJSON(eosWallet.serializeToMap())
        
        var requestResult1 = "\nchangePassword EOS Wallet success:\n"
        requestResult += requestResult1 + prettyPrintJSON(newEosWallet.serializeToMap())
        
        var requestResult2 = "\nExport EOS private Key success:\n"
        requestResult += requestResult2 + "\n active privatekey:" + exportedKeys[0] + "\n owner privatekey:" + exportedKeys[1]
...
//      let wallet = try! identity.delete(password: Constants.password)
